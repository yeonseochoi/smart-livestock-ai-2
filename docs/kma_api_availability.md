...
## 실시간 운영 조건 확인 (2026-09-20 16:06 호출)

| 자료 | 호출 | 결과 | 지연 |
|---|---|---|---:|
| ASOS 시간자료 (전주 146) | `typ01/url/kma_sfctm3.php?tm1=…&tm2=…&stn=146` | 정상, 0.3초, 16:00 관측 수신 | 정시 후 ≤6분 |
| AWS 매분 (익산 702, 2분 범위) | `typ01/cgi-bin/url/nph-aws2_min?tm1=…&tm2=…&stn=702` | 1회 정상(0.5초, 16:04까지), 같은 형태 3회는 30초 후 0행 | 불안정 |
| AWS 매분 (5지점 10분 범위) | 위와 같음, `stn=702:733:763:737:736` | 30초 후 0행 | 실패 |
| ASOS 정시 전지점 | `typ01/url/kma_sfctm2.php` | 403 "활용신청 필요" | 불필요(`sfctm3`로 대체) |

발생 위험 예보(1단)의 기상 입력은 ASOS만으로도 계산되며(`wind_sources.WindField`가 결측 지점을 빼고 가중치를 다시 정규화), AWS가 오지 않는 시각은 ASOS만으로 강등된다(조용한 시각 Hit@5 0.599 → 0.578 수준). 실시간 서비스의 남은 조건은 기상이 아니라 민원 실시간 피드다.
