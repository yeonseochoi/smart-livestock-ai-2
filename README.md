...
| + 기상 | 0.539 |
| + 축산 발생원 노출 | 0.576 |
| + 격자×풍향별 과거 민원율(채택, R5) | 0.581 |
| 위 구성에 바람을 ASOS → ASOS+AWS 격자별 보간으로 | **0.599** |

냄새 유형별로 라벨을 나누면 가축 0.487 → 0.571, 공장 0.643 → 0.748(ASOS 바람 기준)입니다. 공장·하수처리·산업단지 등 외부 시설 데이터는 검토했으나 개선이 없어 채택하지 않았습니다. 186개 격자 중 5개를 찍는 문제이며 과거 빈도만으로도 54%가 나오므로, 발생원·바람의 기여는 그 위 +4~6%p(공장 냄새 +10%p)입니다.
