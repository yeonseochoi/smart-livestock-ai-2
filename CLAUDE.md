...
- 작업 현황은 항상 **기획자에게 설명하듯** 보고한다. 순서는 고정: ① 원래 프로젝트가 하던 일 → ② 어떤 부분을 왜 손보는지 → ③ 구체적으로 어떻게 손보는지(단계별) → ④ 끝나면 남는 산출물 → ⑤ 한계와 미확인 사항. 코드 변경 나열이 아니라 "무엇이 왜 달라지는가"를 말한다.
- 모든 수치는 실제 파일·실행 결과에서 확인한 것만 쓴다. 확인하지 않은 수치는 쓰지 않고 "확인 필요"로 표시한다.
- 임의로 만든 명칭을 쓰지 않는다. 통용되는 용어를 쓰고, 프로젝트 내부 편의상 붙인 이름(예: 작업 갈래 구분)은 처음 쓸 때 "편의상 부르는 이름"임을 밝히고 통용 용어를 병기한다. 예: 소거 실험(ablation study), 인터페이스 계약(interface contract), 테스트 하네스(test harness), 후방 궤적(back trajectory), 수용체 모델(receptor model), 가우시안 플룸(Gaussian plume).
- 새 개념은 일상 비유로 감을 먼저 잡게 한 뒤 정식 용어를 연결하고, 이 프로젝트에서 어떤 의미인지 짧게 붙인다.
